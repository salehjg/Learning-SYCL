...
int main(int argc, char *argv[]) {
    ...
    sycl::queue q{d}; // out-of-order queue
    ...
    auto event3 = q.submit([&](sycl::handler &h) {
        h.depends_on(event1); // Explicit OoO execution dependency.
        h.depends_on(event2); // Explicit OoO execution dependency.
